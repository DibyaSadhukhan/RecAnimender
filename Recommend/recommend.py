...
recommend= flask.Blueprint('recommend', __name__,template_folder='../Templates')

@recommend.route('/from_anime',methods=['POST','GET'])
@recommend.route('/from_anime/uid=<uid>&start=<start>&title=<title>',methods=['POST','GET'])
def recommendation(uid=-1,start=0,title=""):
        ...
        if start+10>len(rec_list[0]):
                ...
                if len(err)==0:
                        ...
                        return flask.redirect(flask.url_for("home"))
@recommend.route('/from_search',methods=['POST','GET'])
